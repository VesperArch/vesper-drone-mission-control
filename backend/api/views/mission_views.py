...
def list_missions(request: Request) -> Response:
    missions = Mission.objects.select_related("assigned_drone").prefetch_related("events").all()
...
def get_mission(request: Request, mission_id: str) -> Response:
    try:
        mission = Mission.objects.select_related("assigned_drone").prefetch_related("events").get(id=mission_id)
...
def create_mission(request: Request) -> Response:
    """
    Validates the payload then delegates to DroneMissionFacade.
    The facade orchestrates ALL subsystems — this view stays thin.
    """
