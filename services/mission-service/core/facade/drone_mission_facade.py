...
───────────────────────
DroneMissionFacade in the microservices context.

The ONLY change from the monolith version:
  self._drone_repo = drone_repo or HttpDroneRepository()  (was DjangoDroneRepository)

Everything else — StartMissionUseCase, the lifecycle thread, _serialize() —
is identical. This proves Clean Architecture + DI make swapping infrastructure
transparent to business logic.

Dependency direction:
  Facade → UseCase → DroneRepository ABC ← HttpDroneRepository (calls drone-service:8001)
"""
...
from infrastructure.repositories.http_drone_repository import HttpDroneRepository
...
class DroneMissionFacade:
    ...
    def __init__(
        ...
        # Microservices injection: drones are remote (HTTP), missions are local (ORM)
        self._drone_repo = drone_repo or HttpDroneRepository()
